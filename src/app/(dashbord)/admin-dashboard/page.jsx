...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    axiosPublic.get('/payments')
      .then(res => {
        setUsers(res.data);
        const successfulPayments = res.data.filter(user => user.status === 'Success');
        const totalSuccessPayments = successfulPayments.reduce((acc, user) => acc + parseFloat(user.price), 0);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Total Payments</h2>
          <p className="text-3xl font-bold text-purple-600">{totalPayments} BDT</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Active Students</h2>

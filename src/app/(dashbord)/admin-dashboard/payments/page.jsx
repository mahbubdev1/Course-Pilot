...
const PaymentHistory = () => {
    ...
    return (
        <div className="p-3 md:p-6 bg-white rounded-lg shadow-sm">
            <h1 className="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6">Payment History</h1>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px]">Txn ID</th>
                                        ...
                                        <tr key={payment._id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-2 py-3 whitespace-nowrap">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-sm ${payment.status === 'Success' ? 'text-green-600' :
                    ...
                    {filteredPayments.length > 0 && (
                        <div className="flex flex-col sm:flex-row justify-between items-center mt-4 px-2 py-3 bg-gray-50 border-t border-gray-200 gap-4">
                            <div className="text-xs sm:text-sm text-gray-700">
                                ...
                                </span> of <span className="font-medium">{filteredPayments.length}</span> results
                            </div>
                            <div className="flex gap-2">
                                <button
                                    onClick={() => paginate(Math.max(1, currentPage - 1))}
                                    disabled={currentPage === 1}
                                    className={`px-2 py-1 sm:px-3 sm:py-1 border border-gray-300 rounded-md text-xs sm:text-sm font-medium ${currentPage === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 hover:bg-gray-50'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`px-2 py-1 sm:px-3 sm:py-1 border border-gray-300 rounded-md text-xs sm:text-sm font-medium ${currentPage === totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 hover:bg-gray-50'
                                        }`}
                                >
                                    Next

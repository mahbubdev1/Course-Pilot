...
const CertificatePage = () => {
    ...
    useEffect(() => {
        axiosPublic.get(`/student-certificate/${user?.email}`)
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                    {
                        certificateData?.map((certificate) => (
                            <div key={certificate._id} className="border rounded-xl shadow-md bg-white">
                                <div id={`capture-${certificate._id}`} className="p-6 bg-white text-center space-y-4 text-sm">
                                    <div>
                                        <h1 className="text-xl font-bold tracking-widest">CERTIFICATE</h1>
                                        <h2 className="text-base font-semibold tracking-wider text-gray-700">OF ACHIEVEMENT</h2>
                                    ...
                                    <div className="text-gray-700 px-4">
                                        Successfully completed <span className="font-semibold">{certificate?.courseTitle}</span>
                                    </div>
                                ...
                                <div className="p-4 border-t flex justify-between items-center">
                                    <p className="text-base font-semibold truncate">{certificate?.courseTitle}</p>
                                    <button

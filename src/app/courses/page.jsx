...
const CourseList = () => {
    ...
    return (
        <div className='pt-[72px] container mx-auto px-4'>
            <h2 className="text-3xl font-semibold text-center text-gray-800 mb-8">Available Courses</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {
                    courses.map((course) => (
                        <div key={course._id} className="border rounded-lg overflow-hidden shadow-lg bg-white transition-all transform hover:scale-105 hover:shadow-xl">
                            ...
                            <div className="p-4 bg-gray-50">
                                <h3 className="font-medium text-gray-900 mb-2 text-lg">{course?.courseTitle}</h3>

                                {/* Duration */}
                                <p className="text-xs text-gray-500 mb-2">Duration: {course?.duration} weeks</p>

                                {/* Course Price */}
                                <div className={`font-semibold text-lg mb-3 ${course?.price === 0 || course?.price === 'Free' ? 'text-green-500' : 'text-blue-600'}`}>
